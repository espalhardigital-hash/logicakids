...
from app.models import FeedbackTypeEnum, FeedbackStatusEnum
...
class ConfiguracionProgresoBase(BaseModel):
    ...
    # pct 80-90, tiempo 0-90) ni lo que los sliders del frontend pueden producir (cant 5-120, tiempo 0-3600).
    ...
    tiempo_default_segundos: Optional[int] = Field(None, ge=0, le=3600)
...
class UXFeedbackBase(BaseModel):
    ...
    comentario: str
    tipo: FeedbackTypeEnum = FeedbackTypeEnum.BUG_VISUAL
    prioridad: str = "media"
...
class UXFeedbackCreate(UXFeedbackBase):
    """Esquema de entrada para crear una nueva anotación UX."""
    pass


class UXFeedbackUpdate(BaseModel):
    """Esquema para que el administrador actualice el estado y notas."""
    estado: Optional[FeedbackStatusEnum] = None
    desarrollador_notes: Optional[str] = None


class UXFeedbackResponse(UXFeedbackBase):
    """Esquema de salida para el cliente / administrador."""
    id: int
    estado: FeedbackStatusEnum
    desarrollador_notes: Optional[str] = None
    fecha_creacion: datetime
    ultima_modificacion: datetime
