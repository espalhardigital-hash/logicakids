...
from .routers import auth_users, ai, feedback, ux_feedback
...
async def lifespan(app: FastAPI):
    ...
    if settings.ENVIRONMENT == "production" or os.environ.get("SKIP_DB_ALTERATIONS", "false").lower() == "true":
        print("=============================================")
        print("⚠️ Entorno de producción / SKIP_DB_ALTERATIONS activo.")
        print("⚠️ Se omite Base.metadata.create_all. Schema gestionado por Alembic.")
        print("=============================================")
    else:
        try:
            async with engine.begin() as conn:
                await conn.run_sync(Base.metadata.create_all)
                print("✅ Tablas de base de datos verificadas/creadas exitosamente (Entorno Local/Dev).")
...
if "*" in raw_origins and settings.SESSION_MODE == "cookie":
    # Browsers fail wildcard CORS when credentials are included
    origins = ["http://localhost:3000", "http://localhost:5173", "http://127.0.0.1:3000", "http://127.0.0.1:5173", "https://logica.espalhar.shop"]
else:
    origins = raw_origins
...
app.include_router(feedback.router, prefix="/admin", tags=["admin"])
app.include_router(ux_feedback.router)
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        print(f"WebSocket inesperadamente cerrado: {e}")
    finally:
