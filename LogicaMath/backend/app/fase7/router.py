...
Router FastAPI ÔÇö Fase 2: Desarrollo Num├®rico y Razonamiento (Refactorizado)
...
Responsabilidades:
  ...
  - Dashboard con los 4 m├│dulos (niveles de pr├íctica y de desaf├¡os).

  - Contenido de teor├¡a din├ímico desde la tabla NivelTeoria.

  - Obtener preguntas (desde BD para pr├íctica libre y desaf├¡os).

  - Validar respuestas:

    - Bucle Espejo (Mirror Loop) en modo Pr├íctica Libre.

    - Salida Temprana (Early Exit) en modo Desaf├¡o con reinicio de progreso.

  - Graduaci├│n a Fase 3 (requiere 26 niveles dominados).
...
# HELPER DE SINCRONIZACI├ôN CON CONFIGURACIONES HEREDADAS (unlockedLevels)
...
# CONSTANTES DE M├ôDULOS Y NIVELES
...
MODULOS_META = {
    ...
    1: {"nombre": "Reconocimiento 3D", "descripcion": "Identificaci├│n de poliedros y vistas 3D.", "icono": "box", "color": "#10B981"},

    2: {"nombre": "Patrones de Crecimiento", "descripcion": "An├ílisis de sucesiones espaciales.", "icono": "bar-chart", "color": "#8B5CF6"},

    3: {"nombre": "Cubos Unitarios", "descripcion": "Modelado del concepto de volumen (u┬│).", "icono": "database", "color": "#F59E0B"},

    4: {"nombre": "Medidas F├¡sicas", "descripcion": "Magnitudes de masa y temperatura.", "icono": "thermometer", "color": "#EC4899"},
...
NIVELES_META = {
    ...
    (1, 1): {"nombre": "Identificaci├│n de poliedros", "descripcion": "V├®rtices, aristas y caras ocultas en formas s├│lidas."},

    (1, 2): {"nombre": "Detecci├│n de bloques", "descripcion": "Detecci├│n de bloques ocultos por perspectivas isom├®tricas."},

    (1, 3): {"nombre": "Moldes desplegados", "descripcion": "Asociaci├│n de moldes desplegados a figuras cerradas 3D."},

    (2, 1): {"nombre": "An├ílisis de sucesiones", "descripcion": "An├ílisis de sucesiones espaciales (Patrones geom├®tricos crecientes)."},

    (2, 2): {"nombre": "Conteo volum├®trico", "descripcion": "Conteo volum├®trico estratificado (Capa por capa)."},

    (2, 3): {"nombre": "Generalizaci├│n", "descripcion": "Generalizaci├│n algebraica de la capa N."},

    (3, 1): {"nombre": "Concepto de volumen", "descripcion": "Modelado del concepto de volumen (u┬│)."},

    (3, 2): {"nombre": "Prismas rectangulares", "descripcion": "C├ílculo anal├¡tico formal de prismas rectangulares (Base x Altura)."},

    (3, 3): {"nombre": "Volumen y l├¡quidos", "descripcion": "Relaci├│n entre volumen c├║bico y l├¡quidos (1 dm┬│ = 1 L)."},

    (4, 1): {"nombre": "Balanzas y term├│metros", "descripcion": "Interpretaci├│n anal├¡tica de balanzas y term├│metros."},

    (4, 2): {"nombre": "Variaciones t├®rmicas", "descripcion": "Variaciones t├®rmicas y comprensi├│n del signo negativo t├®rmico."},

    (4, 3): {"nombre": "La M├íquina Kelvin", "descripcion": "La M├íquina Kelvin: Sumar 273 grados (conversi├│n sin negativos)."},
...
# HELPERS DE NAVEGACI├ôN Y ACCESO
...
def _seccion_operacion(modulo_id: int, nivel_id: int) -> tuple:
    ...
    """Mapea (m├│dulo, nivel) ÔåÆ (secci├│n, operaci├│n) para ConfiguracionProgreso."""
...
async def _get_global_config(db: AsyncSession) -> dict:
    ...
    """Obtiene la configuraci├│n pedag├│gica global de la plataforma desde PlatformSettings."""
...
async def _get_config(db: AsyncSession, seccion: int, operacion: str) -> Optional[ConfiguracionProgreso]:
    ...
    # 1. Intentar obtener configuraci├│n espec├¡fica y activa del bloque/nivel
    ...
    # 2. Fallback: intentar obtener configuraci├│n por defecto de la Fase 2 (seccion = 0, operacion = 'mixta')
...
def _is_nivel_unlocked(progresos: dict, modulo_id: int, nivel_id: int) -> bool:
    ...
    """Verifica si un nivel de pr├íctica libre est├í desbloqueado secuencialmente."""
...
def _is_desafio_unlocked(progresos: dict, modulo_id: int, desafio_id: int, all_practice_approved: bool) -> bool:
    ...
    """Verifica si un desaf├¡o est├í desbloqueado basado en la maestr├¡a de pr├íctica."""
...
# ENDPOINT 1 ÔÇö Dashboard de la Fase 2 (26 niveles)
...
async def get_fase6_dashboard(
    ...
    Devuelve el estado completo de los 4 m├│dulos de Fase 2 para el alumno,

    incluyendo niveles de pr├íctica libre y desaf├¡os.
    ...
    for mod_id in range(1, 5):
        ...
        # 1. Cargar niveles de pr├íctica libre
        ...
        # 2. Cargar desaf├¡os (11, 12, 13)

        desafio_configs = {

            11: {"nombre": "Desaf├¡o 1", "dificultad": "estandar", "tiempo_limite": 30, "max_errores": 3},

            12: {"nombre": "Desaf├¡o 2", "dificultad": "avanzada", "tiempo_limite": 45, "max_errores": 3},

            13: {"nombre": "Desaf├¡o Final", "dificultad": "maestria", "tiempo_limite": 60, "max_errores": 2},
...
# ENDPOINT 2 ÔÇö Contenido de lectura / teor├¡a din├ímico
...
async def get_lectura_fase6(
    ...
    """Devuelve el contenido de lectura/teor├¡a de un nivel espec├¡fico desde la base de datos."""
    ...
    if not theory:
        ...
        raise HTTPException(
            ...
            detail=f"No se encontr├│ contenido te├│rico para el m├│dulo {modulo_id}, nivel {nivel_id}."
...
# ENDPOINT 3 ÔÇö Obtener Pregunta (Pr├íctica con Bucle Espejo y Desaf├¡os aleatorios)
...
async def get_pregunta_fase6(
    ...
    Devuelve la siguiente pregunta para un m├│dulo y nivel (o desaf├¡o) dados.

    Cargado din├ímicamente desde el pool pre-sembrado en la base de datos.

    Soporta Bucle Espejo en pr├íctica libre y selecci├│n aleatoria en desaf├¡os.
    ...
    # 1. MODO DESAF├ìO (modulo_id == 99 o nivel_id en 11, 12, 13)
    ...
    if modulo_id == 99 or nivel_id in (11, 12, 13):
        ...
        if reload:
            ...
            # Borrar los intentos de la tabla general `Intento` para esta secci├│n de desaf├¡o
            ...
            # Restablecer el progreso de maestr├¡a a 0%
        ...
        # Obtener preguntas del desaf├¡o que el alumno ya aprob├│
        ...
        if modulo_id == 99:
            ...
            # Filtrar solo preguntas de nivel de maestr├¡a (Desaf├¡o Final: secciones 1013, 2013, 3013, 4013)

            # Las secciones de desaf├¡os se calculan como m├│dulo*1000+nivel (ej: 1*1000+13=1013)
        ...
        if not preguntas:
            ...
            raise HTTPException(status_code=404, detail="No hay preguntas en el pool para este desaf├¡o.")
        ...
        if not uncompleted:
            ...
            uncompleted = preguntas  # Si aprob├│ todas, permitir repetir
    ...
    # 2. MODO PR├üCTICA LIBRE (1-10)
        ...
        if reload:
            ...
            # 1. Borrar los intentos de la tabla general `Intento` para esta secci├│n
            ...
            # 2. Borrar los intentos de la tabla `IntentoPregunta` para las preguntas de esta secci├│n
            ...
            # 3. Restablecer el progreso de maestr├¡a a 0%
            ...
            # Consultar el ├║ltimo intento en este nivel
        ...
        if latest_attempt and not latest_attempt.es_correcta and latest_attempt.respuesta_dada != "BYPASS_EXPLICACION":
            ...
            if failed_pregunta and failed_pregunta.estructura_padre_id:
                ...
                # Contar cu├íntos intentos lleva en esta misma familia de preguntas
                ...
                # Si lleva menos del m├íximo permitido en el bucle espejo y el ├║ltimo fall├│
        ...
        if espejo_pregunta:
            ...
            # Buscar familias ya tratadas (correctas o con bypass de explicaci├│n)
...
# ENDPOINT 4 ÔÇö Responder pregunta (Valida y actualiza progreso)
...
async def responder_fase6(
    ...
    - Bucle Espejo (Mirror Loop) en modo Pr├íctica Libre (1-10).

    - L├│gica de Salida Temprana (Early Exit) en modo Desaf├¡o (11-13) con reinicio de progreso.
    ...
    # Fase 6 no maneja dinero (m├│dulo 3 = "Cubos Unitarios", no la Tienda de Fase 2/3).

    # Las respuestas son enteros (vol├║menes, litros, gramos, grados) ÔåÆ normalizaci├│n num├®rica simple.
    ...
    if tipo_pregunta == "multiple_opcion":
        ...
        if not payload.alternativa_id:
            ...
            # Caso de timeout o no selecci├│n: tratamos como error
            ...
            feedback_mostrado = "┬íSe acab├│ el tiempo! Intenta responder m├ís r├ípido la pr├│xima vez."
        ...
        if paso_idx < 0 or paso_idx >= len(pasos):
            ...
            raise HTTPException(status_code=400, detail="N├║mero de paso inv├ílido.")
    ...
    if not es_correcta and tipo_pregunta != "multiple_opcion":
        ...
        if pregunta.errores_previstos and isinstance(pregunta.errores_previstos, dict):
            ...
            if not tipo_error:
                ...
                feedback_mostrado = pregunta.errores_previstos.get("calculo", "Revisa tus c├ílculos e int├®ntalo de nuevo.")



    # INTEGRACI├ôN DE INTENTOPREGUNTA E INTENTOPASO (M├ôDULO 4 CONSTRUCTOR)
    ...
    if tipo_pregunta == "constructor_soluciones_chained":
        ...
        # L├│gica de completitud general:

        # Solo si es el ├ÜLTIMO paso y es correcta, aprobamos el IntentoPregunta y mantenemos es_correcta_intento = True
        ...
        if es_correcta and es_ultimo_paso:
            ...
            # Si es un paso intermedio o es incorrecta, la pregunta general no est├í aprobada completa,
    ...
    # 3. ACTUALIZAR PROGRESO Y L├ôGICAS ESPECIALES

    

    # 3.1 MODO DESAF├ìO (11, 12, 13 o Mod 99) -> Salida Temprana (Early Exit)
    ...
    if modulo_id == 99 or nivel_id in (11, 12, 13):
        ...
        if errores_sesion >= max_errores:
            ...
            # Progreso en desaf├¡o: ratio aciertos / cantidad_req (no familias)
...
# ENDPOINT 4.5 ÔÇö Cerrar Rescate (Bypass sin anti-spam)
...
async def cerrar_rescate_fase6(
    ...
    Cierra la explicaci├│n del bloque de rescate y registra un intento virtual 'BYPASS_EXPLICACION'.
    ...
    intento = Intento(
        ...
        feedback_mostrado="Bypass de Explicaci├│n",
...
# ENDPOINT 5 ÔÇö Graduaci├│n a Fase 7 (Exige 24 niveles aprobados)
...
async def graduate_fase6(
    ...
    Grad├║a al alumno de Fase 6 a Fase 7 si todos los 24 niveles (12 de pr├íctica y 12 desaf├¡os) est├ín dominados.
    ...
    if aprobados < 24:
        ...
        raise HTTPException(
            ...
            detail=f"Debes dominar los 24 niveles de Fase 6 (12 de pr├íctica y 12 desaf├¡os). Llevas {aprobados}/24.",
    ...
    if not fase7:
        ...
        raise HTTPException(status_code=500, detail="La Fase 7 a├║n no ha sido configurada.")
    ...
    return {
        ...
        "message": "┬íFelicitaciones! ┬íHas dominado la Fase 6 y avanzas a la Fase 7!",
