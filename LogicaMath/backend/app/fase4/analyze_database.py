"""
Script de auditoría obligatorio de base de datos para la Fase 4.
...
async def run_fase4_audit() -> bool:
    print("=" * 60)
    print("=== AUDITORÍA FASE 4: Operatoria Decimal y Conversiones ===")
    print("=" * 60)
    ...
    async with AsyncSessionLocal() as session:
        ...
        q1 = (
            ...
            .where(Pregunta.fase_id == FASE_DECIMALES_ID)
        ...
        q2 = select(func.count(Pregunta.id)).where(
            and_(Pregunta.fase_id == FASE_DECIMALES_ID, Pregunta.estructura_padre_id.is_(None))
        )
        ...
        q2b = (
            ...
            .where(and_(Pregunta.fase_id == FASE_DECIMALES_ID, Pregunta.seccion < 1000))
            .group_by(Pregunta.seccion)
        )
        sec_fam_counts = (await session.execute(q2b)).all()
        bad_fam_secs = [r for r in sec_fam_counts if r[1] != 120]
        if len(sec_fam_counts) == 12 and len(bad_fam_secs) == 0:
            print("[PASS] 2b Familias de práctica: 12/12 niveles con exactamente 72 familias")
        else:
            print(f"[FAIL] 2b Familias de práctica: {len(bad_fam_secs)} niveles no tienen 72 familias")
            all_passed = False

        # Check 3: Opción múltiple bien formada (4 alternativas, 1 correcta)
        res3 = (await session.execute(
            select(Pregunta.id).where(
                and_(Pregunta.fase_id == FASE_DECIMALES_ID, Pregunta.tipo_pregunta == TipoPreguntaEnum.MULTIPLE_OPCION)
            )
        )).scalars().all()
        ...
        q4 = select(Pregunta).where(Pregunta.fase_id == FASE_DECIMALES_ID)
        ...
        # Check 6: SVG en niveles que lo requieren (módulo 4 de conversiones)
        q6 = select(Pregunta).where(
            and_(Pregunta.fase_id == FASE_DECIMALES_ID, Pregunta.seccion.in_([401, 402, 403]))
        )
        ...
        if missing_svg == 0:
            print(f"[PASS] 6  Figura SVG en niveles requeridos (401,402,403): 0 faltantes ({len(svg_questions)} verificados)")
        else:
        ...
        q7 = select(Pregunta).where(
            and_(Pregunta.fase_id == FASE_DECIMALES_ID, Pregunta.seccion >= 1000)
        )
        ...
        bad_sec_vol = 0
        for p_sec in [101,102,103, 201,202,203, 301,302,303, 401,402,403]:
            if sec_counts.get(p_sec) != 288:
                bad_sec_vol += 1
        
        for c_sec in [1011,1012,1013, 2011,2012,2013, 3011,3012,3013, 4011,4012,4013, 99099]:
            if sec_counts.get(c_sec) != 150:
                bad_sec_vol += 1

        total_questions = sum(sec_counts.values())
        if bad_sec_vol == 0 and total_questions == 5406:
            print(f"[PASS] 9  Volumetría por sección: 25/25 secciones exactas (Total 5.406 preguntas)")
        else:
        ...
        q10 = select(func.count(Pregunta.id)).where(
            and_(Pregunta.fase_id == FASE_DECIMALES_ID, Pregunta.estado == StatusEnum.INACTIVO)
        )
        ...
        q11 = select(ConfiguracionProgreso).where(ConfiguracionProgreso.fase_id == FASE_DECIMALES_ID)
        ...
        if len(cfgs) == 26 and invalid_cfg_count == 0:
            print("[PASS] 11 configuracion_progreso completa: 26/26 filas calibradas correctamente")
        else:
            print(f"[FAIL] 11 configuracion_progreso completa: {len(cfgs)} filas (esperadas 26), {invalid_cfg_count} inválidas")
            all_passed = False

    print("=" * 60)
    if all_passed:
        print("RESULTADO DE AUDITORÍA FASE 4: PASS (100% Invariantes cumplidos)")
        print("=" * 60)
        return True
    else:
        print("RESULTADO DE AUDITORÍA FASE 4: FAIL (Existen violaciones de invariante)")
        print("=" * 60)
        return False

if __name__ == "__main__":
    asyncio.run(run_fase4_audit())
