...
from fastapi import APIRouter, Depends, HTTPException, status, Body, UploadFile, File, Response
from ..config import settings

@router.post("/auth/register", response_model=Token)
async def register(response: Response, user_data: UserRegister, db: AsyncSession = Depends(get_db)):
    try:
...
async def login(response: Response, form_data: OAuth2PasswordRequestForm = Depends(), db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        print(f"Advertencia: No se pudo registrar last_login de {user.username} debido a un error de BD: {db_err}")
        try:
            await db.rollback()
        except Exception as rollback_err:
            print(f"Error crítico: Falló el rollback en el login de {user.username}. Detalle: {rollback_err}")
    access_token = create_access_token(data={"sub": user.id})
    response.set_cookie(
        key="access_token",
        value=access_token,
