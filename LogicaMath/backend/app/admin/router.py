...
async def get_system_config(admin_user: dict = Depends(get_admin_user)):
    if not settings.ENABLE_SYSTEM_CONFIG_ENDPOINT or settings.ENVIRONMENT == "production":
        raise HTTPException(
            status_code=403,
            detail="El endpoint de configuración del sistema está deshabilitado en este entorno."
        )
    env_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), ".env")
    env_vars = dotenv_values(env_path)
    
    raw_db_url = env_vars.get("DATABASE_URL", "")
    masked_db_url = raw_db_url.split("@")[-1] if "@" in raw_db_url else "*****"
    
    return {
        "vps_host": env_vars.get("VPS_HOST", ""),
        "ssh_user": env_vars.get("SSH_USER", ""),
        "database_url": f"postgresql://***:***@{masked_db_url}" if raw_db_url else ""
    }
