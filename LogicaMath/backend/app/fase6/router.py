...
async def _get_config(db: AsyncSession, seccion: int, operacion: str) -> Optional[ConfiguracionProgreso]:
    ...
    result = await db.execute(
        select(ConfiguracionProgreso).where(and_(
            ConfiguracionProgreso.fase_id == FASE6_ID,
...
@router.get("/dashboard", response_model=Fase6Dashboard)
async def get_fase6_dashboard(
    ...
    for mod_id in range(1, 5):
        ...
        for des_id in [11, 12, 13]:
            ...
            mod_porcentaje_total += porcentaje
            desafios.append(Fase6DesafioInfo(
                desafio_id=des_id,
        ...
        modulos.append(Fase6ModuloInfo(
            modulo_id=mod_id,
            nombre=meta["nombre"],
...
@router.get("/lectura/{modulo_id}/nivel/{nivel_id}", response_model=Fase6ContenidoLectura)
...
async def get_pregunta_fase6(
    ...
    if modulo_id == 99 or nivel_id in (11, 12, 13):
        ...
        if pregunta_elex.tipo_pregunta.value == "multiple_opcion" or pregunta_elex.alternativas:
            alts_out = [
                Fase6AlternativaOut(id=alt.id, texto=alt.texto, orden=alt.orden)
        ...
        return Fase6PreguntaParaAlumno(
            id=pregunta_elex.id,
            modulo_id=modulo_id,
            nivel_id=nivel_id,
...
@router.post("/responder", response_model=Fase6ResultadoRespuesta)
async def responder_fase6(
    payload: Fase6ResponderPregunta,
    db: AsyncSession = Depends(get_db),
    alumno: Alumno = Depends(get_current_student),
):
    """
