...
class StorageService:
    ...
    async def upload_feedback_screenshot(self, file_contents: bytes, filename: str) -> str:
        """
        Sube una captura de pantalla de UX Feedback a S3 (MinIO) o localmente en el filesystem como fallback.
        """
        ext = filename.split(".")[-1] if "." in filename else "png"
        unique_name = f"{uuid.uuid4().hex}.{ext}"
        unique_filename = f"screenshots/{unique_name}"
        ...
        if self.s3_client:
            try:
                ...
                logger.error(f"Unexpected S3 Upload error for screenshot: {e}")
                
        # Fallback Local
        local_dir = "app/static/screenshots"
        os.makedirs(local_dir, exist_ok=True)
        local_path = os.path.join(local_dir, unique_name)
    ...
    async def delete_file(self, file_url: str):
        ...
        if "/evaluador/feedback/screenshots/" in file_url:
            try:
                filename = file_url.split("/evaluador/feedback/screenshots/")[-1]
                local_path = os.path.join("app/static/screenshots", filename)
                if os.path.exists(local_path):
                    os.remove(local_path)
                    logger.info(f"Local screenshot deleted: {local_path}")
            except Exception as e:
                logger.error(f"Failed to delete local screenshot file: {e}")
            return
        ...
        if self.s3_client:
            try:
                ...
                # Si el path empieza con el nombre del bucket, remover el prefijo del bucket
