...
import { Fase9Lectura } from './Fase9Types';
...
interface Fase9TheoryModalProps {
  readingData: Fase9Lectura;
  moduleColor: string;
  onClose: () => void;
  onAbort?: () => void;
  userAvatar?: string;
...
export const Fase9TheoryModal: React.FC<Fase9TheoryModalProps> = ({
  readingData,
  moduleColor,
  onClose,
  onAbort,
  ...
  return (
    <div className="f9-reading-overlay">
      <motion.div 
        initial={{ scale: 0.9, y: 20, opacity: 0 }}
        animate={{ scale: 1, y: 0, opacity: 1 }}
        exit={{ scale: 0.9, y: 20, opacity: 0 }}
        className="f9-reading-card flashcard-mode"
      >
        <div className="f9-reading-header">
          <div className="f9-reading-icon" style={{ backgroundColor: `${moduleColor}22`, color: moduleColor, flexShrink: 0 }}>
            <BookOpen size={24} />
          </div>
          <div style={{ flex: 1 }}>
            <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexShrink: 0 }}>
            {onAbort && (
              <button 
                ...
                className="f9-abort-btn"
              >
                <LogOut size={16} />
              </button>
            )}
            <div className="f9-step-indicator" style={{ marginTop: 0 }}>
              Paso {currentStep + 1} de {totalSteps}
            </div>
          </div>
        </div>
        
        <div className="f9-reading-body flashcard-body">
          <AnimatePresence mode="wait" custom={direction}>
            ...
            {currentSlide?.type === 'examples' && (
              ...
              >
                {currentSlide.data.length > 0 ? (
                  <div className="f9-reading-examples">
                    <h3>EJEMPLOS GUIADOS:</h3>
                    {currentSlide.data.map((ex: any, idx: number) => (
                      <div key={idx} className="f9-example-box">
                        <div className="f9-ex-q" dangerouslySetInnerHTML={{ __html: formatContent(ex.enunciado) }} />
                        {ex.pasos ? (
                          <div className="f9-ex-steps">
                            {ex.pasos.map((paso: any) => (
                              <div key={paso.orden} className="f9-ex-step">
                                <span className="f9-ex-step-num">{paso.orden}</span>
                                <span dangerouslySetInnerHTML={{ __html: formatContent(paso.texto) }} />
                              </div>
                            ))}
                          </div>
                        ) : (
                           <div className="f9-ex-legacy">→ <span style={{ color: moduleColor }} dangerouslySetInnerHTML={{ __html: formatContent(ex.respuesta) }} /></div>
                        )}
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="f9-reading-p">No hay ejemplos para este nivel. Avanza al siguiente paso.</div>
                )}
              </motion.div>
            )}
            ...
            {currentSlide?.type === 'interactives' && (
              <motion.div
                ...
                className="f9-flashcard-content"
              >
                <div className="f9-reading-interactive">
                  ...
                  {currentSlide.data.map((int: any, localIdx: number) => {
                    ...
                    return (
                      <div 
                        key={idx} 
                        className={`f9-interactive-box ${isCorrect ? 'correct' : ''} ${feedback[idx] && !isCorrect ? 'error' : ''}`}
                      ...
                      >
                        ...
                        {isLocked ? (
                          ...
                          <>
                              ...
                              <div className="f9-ex-steps">
                                {int.pasos.map((paso: any) => {
                                  ...
                                  if (isInputPaso) {
                                    ...
                                    return (
                                      <div key={paso.orden} className="f9-ex-step input-step">
                                        <span className="f9-ex-step-num">{paso.orden}</span>
                                        <span>{parts[0]} = </span>
                                        <div className="f9-int-input-group">
                                          <input 
                                            type="number" 
                                            className="f9-int-input"
                                            value={answers[idx] || ''}
                            ...
                            {!int.pasos && (
                              <div className="f9-int-input-group legacy">
                                <input 
                                  type="text" 
                                  className="f9-int-input"
                                  value={answers[idx] || ''}
                            ...
                            {feedback[idx] && (
                              <div className={`f9-int-feedback ${feedback[idx].isCorrect ? 'success' : 'error'}`}>
            ...
            {currentSlide?.type === 'tip' && (
              <motion.div
                ...
                className="f9-flashcard-content"
              >
                <div className="f9-reading-tip highlighted">
                ...
                <div className="f9-ready-container">
                  <motion.div 
                    className="f9-ready-rocket"
                    animate={{ 
                      y: [0, -15, 0],
                      rotate: [0, 5, -5, 0]
                    }}
                  ...
                  <div className="f9-ready-msg">
                    ¡Excelente trabajo!<br />
                    Estás listo para la práctica libre.
                  </div>
                  <div className="f9-ready-stars">
                    {[...Array(5)].map((_, i) => (
                      <motion.span 
                        key={i}
                        className="f9-ready-star"
        ...
        <div className="f9-reading-footer">
          <button 
            className="f9-nav-btn" 
          ...
          {currentStep < totalSteps - 1 ? (
            ...
            <button 
              className="f9-reading-close-btn"
