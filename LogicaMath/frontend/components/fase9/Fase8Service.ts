...
export async function getFase8Dashboard(): Promise<Fase8Dashboard> {
  ...
  return fetchDeduplicated(key, async () => {
    const res = await fetchWithTimeout(`${API_URL}/fase8/dashboard`, {
...
export async function submitFase8Answer(
  ...
  const res = await fetchWithTimeout(`${API_URL}/fase8/responder`, {
    method: 'POST',
    headers: getAuthHeaders(),
...
export async function closeFase8Rescate(
  ...
  const res = await fetchWithTimeout(`${API_URL}/fase8/cerrar-rescate`, {
    method: 'POST',
    headers: getAuthHeaders(),
