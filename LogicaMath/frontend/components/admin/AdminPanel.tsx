...
import { Shield, LayoutDashboard, Settings, Activity, Menu, X, LogOut, BookOpen, Server, ArrowLeft, Settings2, MessageSquare } from 'lucide-react';
import './admin.css';
...
import { UXFeedbackTab } from './UXFeedbackTab';
...
type TabType = 'general' | 'pedagogy' | 'performance' | 'content' | 'system' | 'sre' | 'ux_feedback';
...
const AdminPanel: React.FC<Props> = ({ onBack, onLogout }) => {
  ...
  const tabs = [
    ...
    { id: 'ux_feedback', label: 'Buzón de Mejorías UX', icon: MessageSquare },
      ...
      <div className="flex-1 overflow-y-auto relative z-10 p-6 pt-20 md:p-10 custom-scrollbar">
        <div className="max-w-6xl mx-auto w-full">
          <AnimatePresence mode="wait">
            ...
            {activeTab === 'ux_feedback' && (
              <motion.div key="ux_feedback" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.15 }}>
                <ErrorBoundary>
                  <UXFeedbackTab />
                </ErrorBoundary>
              </motion.div>
