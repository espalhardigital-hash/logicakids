...
describe('Fase 5 and Fase 6 API Services', () => {
  it('should fetch Fase 5 dashboard successfully', async () => {
    ...
    expect((result as any).progreso_global).toBe(50);
    expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining('/api/fase5/dashboard'), expect.any(Object));
  });
  ...
  it('should fetch Fase 6 question successfully', async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining('/api/fase6/modulo/1/nivel/1/pregunta'), expect.any(Object));
  });
});
