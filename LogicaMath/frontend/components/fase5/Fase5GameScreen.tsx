...
const Fase5EarlyExitModal: React.FC<{
  moduleColor: string;
  moduloId: number;
  nivelId: number;
  aciertos: number;
...
const Fase5CompletionModal: React.FC<{
  moduloId: number;
  nivelId: number;
  isChallenge: boolean;
  moduleColor: string;
...
const Fase5PhaseGraduationModal: React.FC<{
  studentName: string;
  onClose: () => void;
}> = ({ studentName, onClose }) => {
  // Framer motion variants
...
const Fase5GameScreen: React.FC<Props> = ({ moduloId, nivelId, isEvaluatorMode, onComplete, onBack }) => {
  // 1-22: State Hooks
  const [showSplash, setShowSplash] = useState(true);
  const [pregunta, setPregunta]   = useState<Fase5Pregunta | null>(null);
  ...
  const [readingData, setReadingData] = useState<Fase5Lectura | null>(null);
  ...
  const loadPregunta = useCallback(async (isFirstLoad: boolean = false, resetProgress: boolean = false) => {
    ...
    try {
      ...
      const data = await getFase5Question(moduloId, nivelId, resetProgress);
      ...
      console.error('[Fase5GameScreen] Error loading question', e);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      const resultado = await submitFase5Answer(payload);
      setProgreso({
  ...
  const handleOpenReading = useCallback(async () => {
    ...
    try {
      setError(null);
      const data = await getFase5Reading(moduloId, nivelId);
      setReadingData(data);
      setShowReading(true);
    } catch (e: any) {
      console.error('[Fase5GameScreen] Error loading reading data', e);
  ...
  useEffect(() => {
    ...
    const check = async () => {
      ...
      try {
        ...
        console.error('[Fase5GameScreen] Error checking/loading reading data', e);
        setError('Error al cargar la teoría. Por favor, reintenta.');
      }
    };
    check();
  ...
  const toggleToken = (token: Fase5Token) => {
    ...
    <div className="f5-game-screen">
            ...
            <div className="f5-game-layout-wrap">
              {pregunta.datos_numericos?.geometry && (
                <div className="flex-1 min-w-[300px]">
                  <Fase5ThreeVisualizer payload_tokenizado={pregunta.datos_numericos as any} />
                </div>
              )}
              ...
              >
                ...
                {pregunta.tipo_pregunta === 'multiple_opcion' && (
                  <div className="flex flex-col h-full justify-between">
                     <div className="f5-question-text-box"><div className="f5-question-text" dangerouslySetInnerHTML={{ __html: pregunta.enunciado }} /></div>
      ...
      <AnimatePresence>
        {showReading && readingData && (
          <Fase5TheoryModal readingData={readingData} moduleColor={moduleColor} onClose={() => setShowReading(false)} onAbort={() => isInitialReading ? onBack() : setShowReading(false)} isInitialReading={isInitialReading} isEvaluatorMode={isEvaluatorMode} userAvatar={userAvatar} />
        )}
        {showRescate && feedback.resultado?.explicacion && (
          <Fase5RescateModal explicacion={feedback.resultado.explicacion} moduleColor={moduleColor} onClose={async () => {
            if (pregunta?.id) try { await closeFase5Rescate(moduloId, nivelId, pregunta.id); } catch(e){}
            setShowRescate(false); loadPregunta();
          }} />
        )}
        {showMirrorModal && mirrorPregunta && (
          <Fase5MirrorModal pregunta={mirrorPregunta} moduleColor={moduleColor} lastCorrectAnswer={lastCorrectAnswer} lastQuestionEnunciado={lastQuestionEnunciado} lastWrongAnswer={lastWrongAnswer} onClose={(res) => {
            if (res) {
...
export default Fase5GameScreen;
