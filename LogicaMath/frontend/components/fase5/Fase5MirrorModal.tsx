...
import { Fase5VisualizerEngine } from './Fase5VisualizerEngine';
...
interface Props {
  ...
  moduleColor: string;
  onClose: (result?: Fase5AnswerResult) => void;
...
export const Fase5MirrorModal: React.FC<Props> = ({
  pregunta,
  moduleColor,
  onClose,
  lastCorrectAnswer,
  ...
  if (isPolygonExplanation) {
    ...
    return (
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="f5-feedback-overlay mirror-modal-overlay"
        style={{ zIndex: 1100 }}
      >
        <motion.div
          initial={{ scale: 0.9, y: 20 }}
          animate={{ scale: 1, y: 0 }}
          className="f5-mirror-modal-card glass-card"
          style={{
            maxWidth: '850px',
            width: '95%',
            display: 'flex',
  ...
  const handleSubmit = async (customAnswer?: string) => {
    ...
    try {
      const result = await submitFase5Answer({
        modulo_id: moduloId,
        nivel_id: nivelId,
      ...
      console.error('[Fase5MirrorModal] Error al enviar respuesta:', e);
    }
  };
            ...
            <div className="flex flex-col md:flex-row gap-6 items-center w-full">
              <div className="flex-1 w-full">
                ...
                >
                  ...
                  {showFractionInput ? (
                    <div className="f5-fraction-input-box justify-center my-4 scale-110 origin-center">
                      <div 
                        className={`f5-fraction-field-wrap num ${activeInputField === 'num' ? 'active' : ''}`}
                        onClick={() => !feedback.visible && setActiveInputField('num')}
                      >
                        <span className="f5-fraction-label-indicator">NUMERADOR</span>
                        <div className="f5-fraction-digit-box text-3xl md:text-4xl">
                          {feedback.visible
                            ? feedback.esCorrecta
                      ...
                      <div className="f5-fraction-line-divider" />
                      <div 
                        className={`f5-fraction-field-wrap den ${activeInputField === 'den' ? 'active' : ''}`}
                        onClick={() => !feedback.visible && setActiveInputField('den')}
                      >
                        <span className="f5-fraction-label-indicator">DENOMINADOR</span>
                        <div className="f5-fraction-digit-box text-3xl md:text-4xl">
                          {feedback.visible
                            ? feedback.esCorrecta
                      ...
                      {feedback.visible && (
                        <div className="f5-input-status-elements ml-4">
                          {feedback.esCorrecta ? (
                            <div className="f5-status-badge correct">
                              <svg className="f5-status-icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round">
                                <polyline points="20 6 9 17 4 12" />
                              </svg>
                            </div>
                          ) : (
                            <>
                              <span className="f5-era-pill">
                    ...
                    <div className="f5-numeric-input-wrap mb-2">
                      <div
                        className={`f5-custom-input-box ${
                          feedback.visible
                            ? feedback.esCorrecta
                              ? 'correct'
                              : 'incorrect'
                      ...
                      >
                        <span className="f5-input-value-text text-3xl md:text-4xl">
                          {feedback.visible
                            ? feedback.esCorrecta
