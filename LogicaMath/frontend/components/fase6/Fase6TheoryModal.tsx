...
import { Fase6Lectura } from './Fase6Types';
...
interface Fase6TheoryModalProps {
  readingData: Fase6Lectura;
  moduleColor: string;
  onClose: () => void;
  onAbort?: () => void;
  userAvatar?: string;
...
export const Fase6TheoryModal: React.FC<Fase6TheoryModalProps> = ({
  readingData,
  moduleColor,
  onClose,
  onAbort,
                  ...
                  <div className="f6-reading-examples">
                    ...
                    {currentSlide.data.map((ex: any, idx: number) => (
                      <div key={idx} className="f6-example-box">
                        <div className="f6-ex-q" dangerouslySetInnerHTML={{ __html: formatContent(ex.enunciado) }} />
                        {ex.pasos ? (
                          <div className="f6-ex-steps">
                           ...
                           <div className="f6-ex-legacy">→ <span style={{ color: moduleColor }} dangerouslySetInnerHTML={{ __html: formatContent(ex.respuesta) }} /></div>
                        )}
                      </div>
                    ))}
                  </div>
