...
import type { Fase6Pregunta, Fase6AnswerResult, Fase6AnswerResult as ResultType } from './Fase6Types';
import { submitFase6Answer } from './Fase6Service';

interface Props {
  pregunta: Fase6Pregunta;
  moduleColor: string;
  onClose: (result?: Fase6AnswerResult) => void;
  lastCorrectAnswer?: string;
  lastQuestionEnunciado?: string;
  lastWrongAnswer?: string;
}

export const Fase6MirrorModal: React.FC<Props> = ({ 
  pregunta, 
  moduleColor, 
  onClose,
  lastCorrectAnswer,
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await submitFase6Answer({
        ...
        <div className="flex flex-col md:flex-row gap-6 items-center">
          <div className="flex-1 w-full">
            ...
            >
              ...
              <div className="f5-numeric-input-wrap mb-4">
                <div 
                  className={`f5-custom-input-box ${feedback.visible ? (feedback.esCorrecta ? 'correct' : 'incorrect') : 'focused'}`}
                ...
                >
                  <input
                    ...
                    autoFocus
                  />
                  <span className="f5-input-value-text text-4xl">
                    {feedback.visible 
