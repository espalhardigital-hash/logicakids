...
export async function getFase6Dashboard(): Promise<Fase6Dashboard> {
  ...
  return fetchDeduplicated(key, async () => {
    const res = await fetchWithTimeout(`${API_URL}/fase6/dashboard`, {
...
export async function submitFase6Answer(
  ...
  const res = await fetchWithTimeout(`${API_URL}/fase6/responder`, {
    method: 'POST',
    headers: getAuthHeaders(),
...
export async function closeFase6Rescate(
  ...
  const res = await fetchWithTimeout(`${API_URL}/fase6/cerrar-rescate`, {
    method: 'POST',
    headers: getAuthHeaders(),
...
export async function graduateFase6(): Promise<{
  ...
  const res = await fetchWithTimeout(`${API_URL}/fase6/graduate`, {
    method: 'POST',
    headers: getAuthHeaders(),
  });
