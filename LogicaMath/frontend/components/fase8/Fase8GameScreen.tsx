...
import { Fase8SplitVisualizer } from './Fase8SplitVisualizer';
import { Fase8InteractiveCoordinatePlane } from './Fase8InteractiveCoordinatePlane';
import type {
  Fase8Pregunta,
  Fase8AnswerResult,
  Fase8Token,
...
interface FeedbackState {
  ...
  isError?: boolean;
  errorMessage?: string;
  resultado?: Fase8AnswerResult;
}

const keypadVariants: any = {
...
const Fase8RescateModal: React.FC<{
  explicacion: any;
  moduleColor: string;
  onClose: () => void;
  ...
  return (
    <motion.div 
      ...
      animate={{ opacity: 1 }} 
      exit={{ opacity: 0 }}
      className="f8-feedback-overlay"
      style={{ zIndex: 1000 }}
    >
      <motion.div 
        initial={{ scale: 0.9, y: 20 }}
        animate={{ scale: 1, y: 0 }}
        className="f8-feedback-card rescate glass-card"
        style={{ 
          maxWidth: '550px', 
          width: '90%', 
          padding: '40px',
      ...
      >
        <div className="f8-feedback-emoji" style={{ fontSize: '3rem', marginBottom: '20px' }}>💡</div>
        ...
        <div className="f8-rescate-pasos" style={{ textAlign: 'left', marginBottom: '40px' }}>
...
const Fase8EarlyExitModal: React.FC<{
  moduleColor: string;
  moduloId: number;
  nivelId: number;
  aciertos: number;
  ...
  return (
    ...
    >
      <motion.div 
        initial={{ scale: 0.9, y: 20 }}
        animate={{ scale: 1, y: 0 }}
        className="f8-feedback-card early-exit glass-card"
        style={{ 
          maxWidth: '500px', 
          width: '90%', 
          padding: '40px',
      ...
      >
        <div className="f8-feedback-emoji" style={{ fontSize: '3.5rem', marginBottom: '24px' }}>🛡️</div>
...
const Fase8CompletionModal: React.FC<{
  moduloId: number;
  nivelId: number;
  isChallenge: boolean;
  moduleColor: string;
  ...
  return (
    ...
    >
      <motion.div 
        ...
        initial="hidden"
        animate="show"
        className="f8-feedback-card completion glass-card"
        style={{ 
          maxWidth: '550px', 
          width: '92%', 
          padding: '40px',
...
const Fase8PhaseGraduationModal: React.FC<{
  studentName: string;
  onClose: () => void;
}> = ({ studentName, onClose }) => {
  // Framer motion variants
  ...
  return (
    ...
    >
      <motion.div 
        ...
        initial="hidden"
        animate="show"
        className="f8-feedback-card graduation glass-card"
        style={{ 
          maxWidth: '650px', 
          width: '92%', 
          padding: '45px 35px',
...
const Fase8GameScreen: React.FC<Props> = ({ moduloId, nivelId, isEvaluatorMode, onComplete, onBack }) => {
  // 1-22: State Hooks
  const [showSplash, setShowSplash] = useState(true);
  const [pregunta, setPregunta]   = useState<Fase8Pregunta | null>(null);
  ...
  const [readingData, setReadingData] = useState<Fase8Lectura | null>(null);
  const [userAvatar, setUserAvatar] = useState<string | undefined>(undefined);
  const [showRescate, setShowRescate] = useState(false);
  const [showMirrorModal, setShowMirrorModal] = useState(false);
  const [mirrorPregunta, setMirrorPregunta] = useState<Fase8Pregunta | null>(null);
  ...
  const cartesianConfig = useMemo(() => {
    if (moduloId !== 2 || !pregunta) return null;
    
    const enunciado = pregunta.enunciado || '';
    const respuestaCorrecta = pregunta.respuesta_correcta || '';
    
    const matchA = enunciado.match(/A\((\d+),(\d+)\)/);
    const matchB = enunciado.match(/B\((\d+),(\d+)\)/);
    
    const matchCorrect = respuestaCorrecta.match(/\((\d+),(\d+)\)/);
    ...
    const plainNivel = nivelId > 10 ? nivelId - 10 : nivelId;
    
    if (plainNivel === 1) {
      if (matchCorrect) {
        initialX = parseInt(matchCorrect[1]);
        initialY = parseInt(matchCorrect[2]);
        isInteractive = true;
      }
    } else if (plainNivel === 2) {
      if (matchA && matchCorrect) {
        initialX = parseInt(matchA[1]);
        initialY = parseInt(matchA[2]);
        targetX = parseInt(matchCorrect[1]);
        targetY = parseInt(matchCorrect[2]);
        isInteractive = true;
      }
    } else if (plainNivel === 3) {
      if (matchA && matchB) {
        initialX = parseInt(matchA[1]);
        initialY = parseInt(matchA[2]);
        targetX = parseInt(matchB[1]);
        targetY = parseInt(matchB[2]);
        isInteractive = true;
      }
    }
    
    return isInteractive ? { initialX, initialY, targetX, targetY } : null;
  ...
  const loadPregunta = useCallback(async (isFirstLoad: boolean = false, resetProgress: boolean = false) => {
    ...
    try {
      ...
      console.error('[Fase8GameScreen] Error loading question', e);
  ...
  const handleFeedbackClose = useCallback(() => {
    ...
    if (feedback.esCorrecta) {
      if (pregunta?.tipo_pregunta === 'constructor_soluciones_chained') {
        if ((feedback.resultado as any)?.paso_aprobado === 2 || (feedback.resultado as any)?.paso_approved === 2) {
          loadPregunta();
        } else {
          setTimeout(() => inputRef.current?.focus(), 100);
        }
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      const resultado = await submitFase8Answer(payload);
      setProgreso({
  ...
  const handleOpenReading = useCallback(async () => {
    ...
    try {
      setError(null);
      const data = await getFase8Reading(moduloId, nivelId);
      setReadingData(data);
      setShowReading(true);
    } catch (e: any) {
      console.error('[Fase8GameScreen] Error loading reading data', e);
  ...
  useEffect(() => {
    ...
    const check = async () => {
      ...
      try {
        ...
        console.error('[Fase8GameScreen] Error checking/loading reading data', e);
        setError('Error al cargar la teoría. Por favor, reintenta.');
      }
    };
    check();
  ...
  const toggleToken = (token: Fase8Token) => {
  ...
  return (
    <div className="f8-game-screen">
      ...
      <AnimatePresence>
        {showSplash && (
          <motion.div 
            ...
            className="f8-start-splash-overlay" 
          ...
          >
            {isChallenge ? (
              <motion.div 
                ...
                className="f8-splash-container-premium"
              >
                <div className="f8-splash-badge-premium" style={{ color: moduleColor }}>
                  ¡Desafío Especial!
                </div>
                <h1 className="f8-splash-title-premium">{challengeName}</h1>
                
                {/* Metadatos en cuadrícula premium */}
                <div className="f8-splash-metadata-grid">
                  <div className="f8-splash-meta-card">
                    <div className="f8-splash-meta-icon" style={{ background: `${moduleColor}15` }}>
                      {moduloId === 99 ? (
                    ...
                    <span className="f8-splash-meta-label">Módulo</span>
                    <span className="f8-splash-meta-value">{displayModuleName}</span>
                  </div>
                  ...
                  <div className="f8-splash-meta-card">
                    ...
                    <span className="f8-splash-meta-label">Preguntas</span>
                    <span className="f8-splash-meta-value">{displayQuestionsCount} a superar</span>
                  </div>
                  ...
                  {displayTimeLimit !== "Sin límite" && (
                    <div className="f8-splash-meta-card">
                      ...
                      <span className="f8-splash-meta-label">Tiempo</span>
                      <span className="f8-splash-meta-value">{displayTimeLimit}</span>
                    </div>
                  )}
                </div>

                {/* Animación de cuenta regresiva circular */}
                <div className="f8-splash-countdown-wrapper">
                  <svg className="f8-splash-countdown-svg" viewBox="0 0 100 100">
                    <circle className="f8-splash-countdown-bg" cx="50" cy="50" r="45" />
                    <motion.circle 
                      className="f8-splash-countdown-progress" 
                      cx="50" cy="50" r="45"
                  ...
                  <div className="f8-splash-countdown-number">{countdown}</div>
                </div>

                <div className="f8-splash-hint-premium mt-4">
              ...
              <motion.div 
                ...
                className="f8-splash-content"
              >
                <div className="f8-splash-badge" style={{ color: moduleColor }}>
                  ENTRENAMIENTO LIBRE
                </div>
                <h1 className="f8-splash-title">{moduleName}</h1>
                <div className="f8-splash-level" style={{ background: `${moduleColor}20`, borderColor: `${moduleColor}40` }}>
                  {`NIVEL ${nivelId}`}
                </div>
                <motion.div 
                  animate={{ scale: [1, 1.05, 1], opacity: [0.7, 1, 0.7] }} 
                  transition={{ duration: 1.5, repeat: Infinity }} 
                  className="f8-splash-hint"
                >
                  Toca para comenzar
                </motion.div>
              </motion.div>
      ...
      {loading ? (
        <div className="f8-loading">
          <div className="f8-spinner" style={{ borderTopColor: moduleColor }} />
        ...
        <>
          <AnimatePresence>
            {feedback.visible && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                className={`f8-ambient-glow ${feedback.esCorrecta ? 'correct' : 'incorrect'}`}
              />
            )}
          </AnimatePresence>

          <header className="f8-game-header-modern">
            <button className="f8-header-abort-btn" onClick={onBack} title="Salir del nivel"><IconArrowLeft /></button>
            <div className="f8-header-right-group">
              {isEvaluatorMode && (
                <button 
                  className="f8-view-theory-btn-modern" 
              ...
              <div className="f8-header-badge-pill">
                ...
                <span className="f8-badge-level">FASE 7</span>
                <span className="f8-badge-divider">|</span>
                <span className="f8-badge-level">MÓDULO {moduloId === 99 ? 'MAESTRÍA' : moduloId}</span>
                <span className="f8-badge-divider">|</span>
                <span className="f8-badge-level">NIVEL {nivelId}</span>
                <span className="f8-badge-divider">|</span>
                <span className="f8-badge-challenge">{isChallenge ? 'DESAFÍO' : 'PROGRESO'} {progreso.aciertos}/{maxAciertos}</span>
                {isChallenge && (
                  <>
                    <span className="f8-badge-divider">|</span>
                    <span className="f8-badge-errors animate-pulse" style={{ color: (progreso.intentos - progreso.aciertos) >= maxErroresPermitidos ? '#EF4444' : '#F59E0B', fontWeight: 800 }}>
                      ERRORES: {progreso.intentos - progreso.aciertos}/{maxErroresPermitidos}
                    </span>
                  </>
                )}
                {timer !== null && (
                  <><span className="f8-badge-divider">|</span><span className="f8-badge-timer" style={{ color: timer <= 5 ? '#EF4444' : '#ffffff' }}>{timer}S</span></>
                )}
              </div>
            </div>
            <div className="f8-full-width-progress-bar">
              <div className="f8-full-width-progress-fill" style={{ width: `${barWidth}%`, background: `linear-gradient(90deg, ${moduleColor}80, ${moduleColor})` }} />
            </div>
            {timer !== null && (
              <div className="f8-timer-progress-bar" style={{ position: 'absolute', bottom: 0, left: 0, width: '100%', height: '3px', background: 'rgba(255,255,255,0.04)', overflow: 'hidden' }}>
                <div className="f8-full-width-progress-fill" style={{ width: `${(timer / maxTimer) * 100}%`, background: timer <= 5 ? '#EF4444' : 'linear-gradient(90deg, #3B82F6, #10B981)', height: '100%' }} />
              </div>
            )}
          </header>
          ...
          <main className="f8-game-body">
            <div className="f8-game-layout-wrap">
              {cartesianConfig ? (
                <div className="flex-1 min-w-[300px] flex justify-center items-center my-4">
                  <Fase8InteractiveCoordinatePlane
                    initialX={cartesianConfig.initialX}
                    initialY={cartesianConfig.initialY}
                    targetX={cartesianConfig.targetX}
                    targetY={cartesianConfig.targetY}
                    moduleColor={moduleColor}
                    onChange={(x, y) => {
                      if (pregunta.tipo_pregunta === 'multiple_opcion' && pregunta.alternativas) {
                        const formatted = `(${x},${y})`;
                        const match = pregunta.alternativas.find(alt => alt.texto.replace(/\s+/g, '') === formatted);
                        if (match) {
                          setSelectedAltId(match.id);
                        } else {
                          setSelectedAltId(null);
                        }
                      } else {
              ...
              >
                ...
                {pregunta.tipo_pregunta === 'respuesta_numerica' && (
                  <div className="flex flex-col h-full justify-between">
                    <div className="f8-question-text-box"><div className={(pregunta.enunciado || '').length < 25 ? "f8-question-text short" : "f8-question-text"} dangerouslySetInnerHTML={safeHtml(pregunta.enunciado)} /></div>
                    <div className="f8-numeric-input-wrap">
                      <div className={`f8-custom-input-box ${feedback.visible ? (feedback.esCorrecta ? 'correct' : 'incorrect') : 'focused'}`} onClick={() => inputRef.current?.focus()}>
                        <input ref={inputRef} type="text" value={respuesta} onChange={e => !feedback.visible && /^[0-9,.\-]*$/.test(e.target.value) && setRespuesta(e.target.value)} onKeyDown={handleKeyDown} className="f8-hidden-input" autoFocus autoComplete="off" inputMode="none" />
                        <span className="f8-input-value-text">{feedback.visible ? (feedback.esCorrecta ? (feedback.resultado?.respuesta_correcta || respuesta) : (respuesta || '?')) : (respuesta || '?')}</span>
                        {feedback.visible && (
                          <div className="f8-input-status-elements">
                            {feedback.esCorrecta ? <div className="f8-status-badge correct"><svg className="f8-status-icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="4"><polyline points="20 6 9 17 4 12" /></svg></div> :
                              <><span className="f8-era-pill">Era: {feedback.resultado?.respuesta_correcta}</span><div className="f8-status-badge incorrect"><svg className="f8-status-icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="4"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg></div></>
                            }
                          </div>
                        )}
                      </div>
                    ...
                    {!isChallenge && <div className="f8-scores-container"><div className="f8-score-box correct"><span className="f8-score-label">CORRECTAS</span><span className="f8-score-value">{progreso.aciertos}</span></div><div className="f8-score-box incorrect"><span className="f8-score-label">ERRORES</span><span className="f8-score-value">{feedback.resultado?.errores_sesion ?? (progreso.intentos - progreso.aciertos)}</span></div></div>}
                  </div>
                )}
                {/* Otros tipos (constructor, tokens, etc) simplificados para brevedad pero funcionales */}
                {pregunta.tipo_pregunta === 'multiple_opcion' && (
                  <div className="flex flex-col h-full justify-between">
                     <div className="f8-question-text-box"><div className="f8-question-text" dangerouslySetInnerHTML={safeHtml(pregunta.enunciado)} /></div>
                     <div className="grid gap-3 mt-6">
                       {pregunta.alternativas?.map(alt => (
                         <button key={alt.id} disabled={feedback.visible} onClick={() => setSelectedAltId(alt.id)}
                           className={`f8-mc-option-btn ${selectedAltId === alt.id ? 'selected' : ''}`}
                ...
                {pregunta.tipo_pregunta === 'constructor_soluciones_chained' && (
                  <div className="flex flex-col h-full justify-between gap-4">
                    <div className="f8-question-text-box">
                      <div className="f8-question-text" dangerouslySetInnerHTML={safeHtml(cleanEnunciado(pregunta.enunciado))} />
                    </div>
                    ...
                    <div className="flex flex-col gap-4 my-2">
                      ...
                      >
                        ...
                        {paso === 1 ? (
                          <div className={`f8-custom-input-box focused ${feedback.visible ? (feedback.esCorrecta ? 'correct' : 'incorrect') : ''}`} onClick={() => inputRef.current?.focus()}>
                            <input 
                              ref={inputRef} 
                              type="text" 
                              value={respuesta} 
                            ...
                            {feedback.visible && (
                              <div className="f8-input-status-elements">
                                {feedback.esCorrecta ? (
                                  <div className="f8-status-badge correct">
                                    <svg className="f8-status-icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="4"><polyline points="20 6 9 17 4 12" /></svg>
                                  </div>
                                ) : (
                                  <>
                                    <span className="f8-era-pill">Era: {feedback.resultado?.respuesta_correcta}</span>
                                    <div className="f8-status-badge incorrect">
                                      <svg className="f8-status-icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="4"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>
                                    </div>
                                  </>
                                )}
                              </div>
                          ...
                          <div className="f8-custom-input-box correct opacity-80 pointer-events-none">
                            <span className="f8-input-value-text">{paso1Valor}</span>
                            <div className="f8-input-status-elements">
                              <div className="f8-status-badge correct">
                                <svg className="f8-status-icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="4"><polyline points="20 6 9 17 4 12" /></svg>
                              </div>
        ...
        {showSplash && (
          ...
          >
            {isChallenge ? (
              ...
              >
                <div className="f8-splash-badge-premium" style={{ color: moduleColor, fontSize: '1.2rem', fontWeight: 800, letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '10px' }}>
                  ZONA DE DESAFÍO
                </div>
                <h1 className="f8-splash-title-premium" style={{ fontSize: '2.5rem', fontWeight: 900, color: '#fff', marginBottom: '30px' }}>{challengeName}</h1>
                
                <div className="f8-splash-metadata-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '15px', marginBottom: '40px' }}>
                  <div className="f8-splash-meta-card" style={{ background: 'rgba(255,255,255,0.05)', padding: '20px', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.1)' }}>
                ...
                <div className="f8-splash-countdown-wrapper" style={{ position: 'relative', width: '120px', height: '120px', margin: '0 auto' }}>
              ...
              >
                <div className="f8-splash-badge" style={{ color: moduleColor, fontSize: '1.2rem', fontWeight: 800, letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '15px' }}>
                  PRÁCTICA LIBRE
                </div>
                <h1 className="f8-splash-title" style={{ fontSize: '3rem', fontWeight: 900, color: '#fff', marginBottom: '20px' }}>{moduleName}</h1>
                <div className="f8-splash-level" style={{ display: 'inline-block', padding: '8px 24px', background: `${moduleColor}20`, border: `1px solid ${moduleColor}40`, borderRadius: '30px', color: moduleColor, fontWeight: 800, fontSize: '1.2rem', marginBottom: '40px' }}>
                  Nivel {nivelId}
                </div>
                <div className="f8-splash-hint" style={{ opacity: 0.6 }}>
      ...
      <AnimatePresence>
        {showReading && readingData && (
          <Fase8TheoryModal readingData={readingData} moduleColor={moduleColor} onClose={() => setShowReading(false)} onAbort={() => isInitialReading ? onBack() : setShowReading(false)} isInitialReading={isInitialReading} isEvaluatorMode={isEvaluatorMode} userAvatar={userAvatar} />
        )}
        {showRescate && feedback.resultado?.explicacion && (
          <Fase8RescateModal explicacion={feedback.resultado.explicacion} moduleColor={moduleColor} onClose={async () => {
            if (pregunta?.id) try { await closeFase8Rescate(moduloId, nivelId, pregunta.id); } catch(e){}
            setShowRescate(false); loadPregunta();
          }} />
        )}
        {showMirrorModal && mirrorPregunta && (
          <Fase8MirrorModal pregunta={mirrorPregunta} moduleColor={moduleColor} lastCorrectAnswer={lastCorrectAnswer} lastQuestionEnunciado={lastQuestionEnunciado} lastWrongAnswer={lastWrongAnswer} onClose={(res) => {
            if (res) {
        ...
        {showEarlyExit && (
          <Fase8EarlyExitModal 
            ...
            onClose={() => { setShowEarlyExit(false); onBack(); }} 
          />
        )}
        {showGraduation && (
          <Fase8PhaseGraduationModal studentName={studentName} onClose={() => { setShowGraduation(false); navigate('/map'); }} />
        )}
        {showCompletion && (
          <Fase8CompletionModal 
            moduloId={moduloId}
            nivelId={nivelId}
...
export default Fase8GameScreen;
