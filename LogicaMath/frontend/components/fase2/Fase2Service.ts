...
export async function getFase2Dashboard(): Promise<Fase2Dashboard> {
  ...
  return fetchDeduplicated(key, async () => {
    const res = await fetchWithTimeout(`${API_URL}/fase2/dashboard`, {
...
export async function closeFase2Rescate(
  ...
  const res = await fetchWithTimeout(`${API_URL}/fase2/cerrar-rescate`, {
    method: 'POST',
    headers: getAuthHeaders(),
...
export async function graduateFase2(): Promise<{
  ...
  const res = await fetchWithTimeout(`${API_URL}/fase2/graduate`, {
    method: 'POST',
    headers: getAuthHeaders(),
  });
