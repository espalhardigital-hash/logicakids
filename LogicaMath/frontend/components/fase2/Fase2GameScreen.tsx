...
import { getFase2Question, submitFase2Answer, getFase2Reading, closeFase2Rescate, graduateFase2 } from './Fase2Service';
...
const Fase2GameScreen: React.FC<Props> = ({ moduloId, nivelId, isEvaluatorMode, onComplete, onBack }) => {
  ...
  const loadPregunta = useCallback(async (isFirstLoad: boolean = false, resetProgress: boolean = false) => {
    ...
    try {
      ...
      console.error('[Fase2GameScreen] Error loading question', e);
  ...
  const handleOpenReading = useCallback(async () => {
    ...
    try {
      ...
      console.error('[Fase2GameScreen] Error loading reading data', e);
  ...
  useEffect(() => {
    ...
    const check = async () => {
      ...
      try {
        ...
        console.error('[Fase2GameScreen] Error checking/loading reading data', e);
        setError('Error al cargar la teoría. Por favor, reintenta.');
      }
    };
    check();
    ...
    <div className="f2-game-screen">
      ...
      <AnimatePresence>
        {showReading && readingData && (
          <Fase2TheoryModal readingData={readingData} moduleColor={moduleColor} onClose={() => setShowReading(false)} onAbort={() => isInitialReading ? onBack() : setShowReading(false)} isInitialReading={isInitialReading} isEvaluatorMode={isEvaluatorMode} userAvatar={userAvatar} />
        )}
