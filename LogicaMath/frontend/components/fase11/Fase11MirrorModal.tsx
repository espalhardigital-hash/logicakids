...
import type { Fase11Pregunta, Fase11AnswerResult, Fase11AnswerResult as ResultType } from './Fase11Types';
import { submitFase11Answer } from './Fase11Service';

interface Props {
  pregunta: Fase11Pregunta;
  moduleColor: string;
  onClose: (result?: Fase11AnswerResult) => void;
  lastCorrectAnswer?: string;
  lastQuestionEnunciado?: string;
  lastWrongAnswer?: string;
}

export const Fase11MirrorModal: React.FC<Props> = ({ 
  pregunta, 
  moduleColor, 
  onClose,
  lastCorrectAnswer,
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await submitFase11Answer({
  ...
  return (
    <motion.div 
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="f11-feedback-overlay mirror-modal-overlay"
      style={{ zIndex: 1100 }}
    >
      <motion.div 
        initial={{ scale: 0.9, y: 20 }}
        animate={{ scale: 1, y: 0 }}
        className="f11-mirror-modal-card glass-card"
        style={{ 
          maxWidth: '800px', 
          width: '95%',
          display: 'flex',
        ...
        <div className="flex flex-col md:flex-row gap-6 items-center">
          <div className="flex-1 w-full">
            ...
            >
              ...
              <div className="f11-numeric-input-wrap mb-4">
                <div 
                  className={`f11-custom-input-box ${feedback.visible ? (feedback.esCorrecta ? 'correct' : 'incorrect') : 'focused'}`}
                ...
                >
                  <input
                    ...
                    className="f11-hidden-input"
                    autoFocus
                  />
                  <span className="f11-input-value-text text-4xl">
                    {feedback.visible 
                      ? (feedback.esCorrecta ? (feedback.resultado?.respuesta_correcta || respuesta) : (respuesta || '?')) 
                      : (respuesta || '?')}
                  </span>
                  {feedback.visible && (
                    <div className="f11-input-status-elements">
                      {feedback.esCorrecta ? (
                        <div className="f11-status-badge correct">
                          <svg className="f11-status-icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round">
                            <polyline points="20 6 9 17 4 12" />
                          </svg>
                        </div>
                      ) : (
                        <>
                          <span className="f11-era-pill">Era: {feedback.resultado?.respuesta_correcta}</span>
                          <div className="f11-status-badge incorrect">
                            <svg className="f11-status-icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round">
