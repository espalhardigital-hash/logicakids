...
import { Fase11Lectura } from './Fase11Types';
import { formatContent, fixEncoding } from '../../services/textService';
import { getAvatarUrl } from '../../services/storageService';
import './Fase11Styles.css';

interface Fase11TheoryModalProps {
  readingData: Fase11Lectura;
  moduleColor: string;
  onClose: () => void;
  onAbort?: () => void;
  userAvatar?: string;
...
export const Fase11TheoryModal: React.FC<Fase11TheoryModalProps> = ({
  readingData,
  moduleColor,
  onClose,
  onAbort,
  ...
  return (
    <div className="f11-reading-overlay">
      <motion.div 
        initial={{ scale: 0.9, y: 20, opacity: 0 }}
        animate={{ scale: 1, y: 0, opacity: 1 }}
        exit={{ scale: 0.9, y: 20, opacity: 0 }}
        className="f11-reading-card flashcard-mode"
      >
        <div className="f11-reading-header">
          <div className="f11-reading-icon" style={{ backgroundColor: `${moduleColor}22`, color: moduleColor, flexShrink: 0 }}>
            <BookOpen size={24} />
          </div>
          <div style={{ flex: 1 }}>
            <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexShrink: 0 }}>
            {onAbort && (
              <button 
                ...
                className="f11-abort-btn"
              >
                <LogOut size={16} />
              </button>
            )}
            <div className="f11-step-indicator" style={{ marginTop: 0 }}>
              Paso {currentStep + 1} de {totalSteps}
            </div>
          </div>
        </div>
        
        <div className="f11-reading-body flashcard-body">
          <AnimatePresence mode="wait" custom={direction}>
            {currentSlide?.type === 'intro' && (
              <motion.div
                ...
                className="f11-flashcard-content"
              >
                {readingData.parrafos.map((p, idx) => (
                  <p key={idx} className="f11-reading-p" dangerouslySetInnerHTML={{ __html: formatContent(p) }} />
                ))}

                {readingData.diccionario && Object.keys(readingData.diccionario).length > 0 && (
                  <div className="f11-reading-dictionary">
                    <h3>📖 EL DICCIONARIO DEL NIVEL:</h3>
                    <div className="f11-dict-grid">
                      {Object.entries(readingData.diccionario).map(([termino, definicion], idx) => (
                        <div key={idx} className="f11-dict-card" style={{ borderColor: `${moduleColor}55` }}>
                          <div className="f11-dict-term" style={{ color: moduleColor }} dangerouslySetInnerHTML={{ __html: formatContent(termino) }} />
                          <div className="f11-dict-def" dangerouslySetInnerHTML={{ __html: formatContent(definicion as string) }} />
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            {currentSlide?.type === 'examples' && (
              ...
              >
                {currentSlide.data.length > 0 ? (
                  <div className="f11-reading-examples">
                    <h3>EJEMPLOS GUIADOS:</h3>
                    {currentSlide.data.map((ex: any, idx: number) => (
                      <div key={idx} className="f11-example-box">
                        <div className="f11-ex-q" dangerouslySetInnerHTML={{ __html: formatContent(ex.enunciado) }} />
                        {ex.pasos ? (
                          <div className="f11-ex-steps">
                            {ex.pasos.map((paso: any) => (
                              <div key={paso.orden} className="f11-ex-step">
                                <span className="f11-ex-step-num">{paso.orden}</span>
                                <span dangerouslySetInnerHTML={{ __html: formatContent(paso.texto) }} />
                              </div>
                            ))}
                          </div>
                        ) : (
                           <div className="f11-ex-legacy">→ <span style={{ color: moduleColor }} dangerouslySetInnerHTML={{ __html: formatContent(ex.respuesta) }} /></div>
                        )}
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="f11-reading-p">No hay ejemplos para este nivel. Avanza al siguiente paso.</div>
                )}
              </motion.div>
            )}
            ...
            {currentSlide?.type === 'interactives' && (
              <motion.div
                ...
                className="f11-flashcard-content"
              >
                <div className="f11-reading-interactive">
                  ...
                  {currentSlide.data.map((int: any, localIdx: number) => {
                    ...
                    return (
                      <div 
                        key={idx} 
                        className={`f11-interactive-box ${isCorrect ? 'correct' : ''} ${feedback[idx] && !isCorrect ? 'error' : ''}`}
                      ...
                      >
                        ...
                        {isLocked ? (
                          ...
                          <>
                              ...
                              <div className="f11-ex-steps">
                                {int.pasos.map((paso: any) => {
                                  ...
                                  if (isInputPaso) {
                                    ...
                                    return (
                                      <div key={paso.orden} className="f11-ex-step input-step">
                                        <span className="f11-ex-step-num">{paso.orden}</span>
                                        <span>{parts[0]} = </span>
                                        <div className="f11-int-input-group">
                                          <input 
                                            type="number" 
                                            className="f11-int-input"
                                            value={answers[idx] || ''}
                            ...
                            {!int.pasos && (
                              <div className="f11-int-input-group legacy">
                                <input 
                                  type="text" 
                                  className="f11-int-input"
                                  value={answers[idx] || ''}
                            ...
                            {feedback[idx] && (
                              <div className={`f11-int-feedback ${feedback[idx].isCorrect ? 'success' : 'error'}`}>
            ...
            {currentSlide?.type === 'tip' && (
              <motion.div
                ...
                className="f11-flashcard-content"
              >
                <div className="f11-reading-tip highlighted">
                ...
                <div className="f11-ready-container">
                  <motion.div 
                    className="f11-ready-rocket"
                    animate={{ 
                      y: [0, -15, 0],
                      rotate: [0, 5, -5, 0]
                    }}
                  ...
                  <div className="f11-ready-msg">
                    ¡Excelente trabajo!<br />
                    Estás listo para la práctica libre.
                  </div>
                  <div className="f11-ready-stars">
                    {[...Array(5)].map((_, i) => (
                      <motion.span 
                        key={i}
                        className="f11-ready-star"
        ...
        <div className="f11-reading-footer">
          <button 
            className="f11-nav-btn" 
          ...
          {currentStep < totalSteps - 1 ? (
            ...
            <button 
              className="f11-reading-close-btn"
