...
export function validarYCorregirParametros({
  ...
  if (!admiteDecimales && tipoObj === "discreto" && respuestaBruta % 1 !== 0) {
    ...
    if (!encontrado) {
      // Fallback matemático: Forzar a que cantidadCorregida sea un múltiplo del divisor para asegurar división entera
      const factorFallback = Math.ceil(cantidadPorCaja / divisor);
      cantidadCorregida = (factorFallback > 0 ? factorFallback : 1) * divisor;
    }
  ...
  if (tipo === "dos_pasos" && enunciadoTemplate) {
    const partes = enunciadoTemplate.split(/(?<=[.?!])(?:\s+|\n+)/);
    if (partes.length > 1) {
