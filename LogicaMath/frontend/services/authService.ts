...
export const logout = async (): Promise<void> => {
  try {
    await fetchWithTimeout(`${API_URL}/auth/logout`, { method: 'POST' });
  } catch {
...
/**
 * Get current user from storage or fetch from API (supports cookies or Bearer token)
 */
export const getCurrentUser = async (): Promise<any | null> => {
  ...
  try {
    const response = await fetchWithTimeout(`${API_URL}/users/me`, { headers });
