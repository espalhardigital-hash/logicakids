...
> Este documento describe la arquitectura global y teórica del Backend. Para ver el paso a paso exacto, los contratos de datos y los esqueletos de código para construir una fase desde cero, consulta el manual táctico:
...
* **Compatibilidad Retrospectiva y Fallbacks**:
  ...
  - **Migración de Datos Semilla**: El script de semilla (`seed.py`) incorpora la rutina `migrar_datos_fase1_legacy` que duplica automáticamente la maestría y progreso de los alumnos que poseían aprobación en la sección legacy `1` de una operación hacia todas las nuevas secciones dinámicas.

## 3. Paso 1: Definición del Modelo y Base de Datos (Arquitectura de Modelos Aislados)

Para maximizar el desacoplamiento, evitar conflictos de migraciones (Alembic) y permitir que cada fase evolucione de manera independiente, **el sistema exige el uso de tablas físicas segmentadas por fase** (ej. `intento_preguntas_faseX`, `nivel_teoria_faseX`).
En lugar de una única tabla global y masiva, cada fase define sus propios modelos en su respectivo archivo `backend/app/faseX/models.py`.

### Diagrama Entidad-Relación (Arquitectura Aislada por Fase)

```mermaid
erDiagram
    ALUMNO ||--o{ PROGRESO_MAESTRIA_FASEX : "tiene"
    ALUMNO ||--o{ INTENTO_PREGUNTAS_FASEX : "registra"
    
    PROGRESO_MAESTRIA_FASEX {
        int id PK
        int alumno_id FK
        int modulo_id
        int nivel_id
...
> **Regla de identificación:** Los modelos SQLAlchemy de la Fase X deben declarar explícitamente su nombre de tabla aislando su contexto para no colisionar con otras fases, por ejemplo: `__tablename__ = "intento_preguntas_faseX"`.
...
* `ejemplo_guiado`: JSONB de ejemplos resueltos paso a paso (mínimo 5 ejemplos para Fases 1–3; para la **Fase 4** la norma se ajusta a **exactamente 4 ejemplos guiados con TJS de 5 pasos**, según Derogación A0 #1). Palabras clave destacadas mediante `.keyword-highlight` (`<span class="keyword-highlight">...</span>`).
