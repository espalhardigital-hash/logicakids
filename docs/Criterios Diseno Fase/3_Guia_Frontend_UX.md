...
> Si vas a **programar la UI de una nueva fase**, no necesitas deducir los estilos desde cero. Usa los bloques de código exactos de Framer Motion, animaciones y CSS que se encuentran en el manual táctico:
> 👉 [guia_creacion_fase.md](../../../guia_creacion_fase.md)
...
### 2.3. Reglas Innegociables de Viewport y Presupuesto de Contenido (T3 & T4)

1. **T3 — Cero Scroll Vertical (Regla Innegociable):** Ninguna pantalla de teoría, práctica o desafío debe generar scrollbar vertical. Toda la información del paso activo debe caber completamente dentro del marco visible sin necesidad de desplazamiento.
2. **T4 — Ventana Fija de 950 × 620 px:** El contenedor principal de las tarjetas de juego se diseña sobre un viewport fijo objetivo de **950 px de ancho por 620 px de alto**.
3. **Dispositivos Soportados:** Desktop (laptops y pantallas de escritorio) y Tablet Horizontal. **Dispositivos móviles (smartphones) quedan excluidos** del alcance de maquetación principal.
4. **Presupuesto Contable por Paso (§4.3):**
   - Enunciados en teoría: máximo 3 líneas de texto (~35 palabras).
   - Enunciados en desafíos: objetivo ~30 palabras (límite duro de 40 palabras).
   - Ilustraciones y gráficos SVG: alto máximo de **140 px**.
   - Opciones múltiples: máximo 4 alternativas con texto paralelo de 1 a 2 líneas.
   - Regla de ajuste: Ante exceso de contenido, **se reduce la cantidad de información por paso**, NUNCA el tamaño de la letra.
5. **Contadores por Bloque (§4.5):** Los contadores de progreso se presentan fragmentados por bloque activo en lugar de un contador global estático.
6. **Pasos con Elección:** Se admite el tipo de paso con opciones de elección narrativa y revelación posterior de contenido (C2.4bis).
...
### 3.3. Estado Global y Gestión de Estados del Juego

La arquitectura del frontend admite dos enfoques válidos para la orquestación del flujo de juego (pregunta, respuesta, feedback):

1. **Gestión con Estado Local (Recomendado por Consistencia):** Consiste en centralizar el ciclo de vida (Timers, Inputs, APIs) en el componente contenedor principal (ej: `FaseXGameScreen.tsx`) utilizando `useState`, `useRef` y `useEffect`. Este es el patrón implementado en la **Fase 2 (Producción)** y sirve como plantilla de referencia óptima para facilitar la clonación segura de código en nuevas fases.
2. **Gestión con Estado Global (Zustand):** Adecuado para flujos desacoplados donde múltiples pantallas independientes requieran leer o modificar la sesión activa.

**Regla de Oro Invariable:** Independientemente del enfoque de estado elegido (Zustand o local), el frontend **nunca** debe calcular o asumir el progreso académico o la validez de la respuesta. Siempre debe actuar como renderizador fiel de las respuestas en JSON devuelvas por el backend (modelo Server-Authoritative).
...
#### 5.1.2. UX de Preguntas Encadenadas (Chained Steps)

Para problemas matemáticos de múltiples pasos (siguiendo el patrón del Módulo 4 de Fase 2), se utiliza la UX de **Constructor de Soluciones Encadenadas**:
1. **Paso a Paso Secuencial:** El input del "Paso 2" permanece oculto o deshabilitado visualmente hasta que el alumno haya ingresado una respuesta en el "Paso 1".
2. **Congelamiento de Variables:** Al avanzar al siguiente paso, el valor ingresado en el paso anterior se "congela" visualmente (cambia a un bloque de texto inerte tipo etiqueta), sirviendo como memoria de trabajo para el alumno.
3. **Evaluación Integral:** Al enviar la respuesta final, la animación del Bucle Espejo (shake rojo) se aplica a todo el contenedor de pasos para indicar que la secuencia fue errónea.
