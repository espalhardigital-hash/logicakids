...
> Este tomo es puramente teórico y conceptual. Si estás a punto de **escribir código para implementar una nueva fase**, detente y utiliza el manual táctico interactivo:
...
* **Ejemplos Guiados (Mínimo 5):** Cada nivel de cada módulo en todas las fases debe garantizar por lo menos 5 ejemplos guiados completamente resueltos paso a paso para ofrecer una cobertura didáctica y una variedad de modelado óptima.
  > 🔴 **DEROGACIÓN NORMATIVA (A0 #1 — Fase 4):** Para la Fase 4 (Decimales), la norma de "mínimo 5 ejemplos guiados" se ajusta a **exactamente 4 ejemplos guiados por nivel**. Justificación: El nuevo formato de TJS guiado consta de 5 pasos estructurados (compromiso, aislamiento, modelo, andamiaje, transferencia) en lugar de un paso único, por lo que 4 ejemplos con TJS generan mayor volumen interactivo y densidad de aprendizaje que 5 ejemplos pasivos tradicionales.
...
### 4.1. Formato de Interfaz y Tipos de Resolución

La práctica libre exige exclusivamente cuadros de texto (`input`) vacíos o constructores encadenados. Se prohíbe de forma absoluta e innegociable el uso de opción múltiple en la práctica libre de todas las fases. La opción múltiple queda restringida únicamente a la Zona de Desafíos (D1/D2) o pasos teóricos con elección (C2.4bis).

Para abarcar la complejidad matemática, la plataforma soporta diferentes mecánicas de resolución, tomando siempre como **referencia de implementación la Fase 2**:

1. **Resolución Atómica (Simple):** El alumno ingresa un único valor final y el backend lo evalúa.
2. **Constructor de Soluciones (Preguntas Encadenadas / Chained Steps):** Para problemas de múltiples pasos (ej. divisiones complejas), el frontend debe permitir que el alumno resuelva el "Paso 1". Una vez ingresado, el valor se congela visualmente en pantalla (`valor_paso1_congelado`) y se habilita el input del "Paso 2". Esta UX es fundamental para guiar el proceso cognitivo sin abrumar al estudiante.
3. **Interacción Textual (Sistema de Tokens):** Cuando una pregunta exige interacción textual, como subrayado o selección de datos relevantes (ej. Legado Módulo Detective), el frontend debe enviar IDs de tokens (`FaseXToken`) y no texto crudo.

Ejemplo de Payload con Tokens:
