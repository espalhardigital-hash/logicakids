...
The system SHALL require the user to explicitly click a confirmation button after selecting a multiple choice alternative in Phase 4, matching the UI pattern from Phase 2. The pool of alternatives SHALL now include distractors generated from a wider range of numerical permutations, producing more varied incorrect options.
...
#### Scenario: Diverse incorrect alternatives
- **WHEN** the system generates multiple choice alternatives for a challenge question
- **THEN** at least 2 of the 3 incorrect alternatives SHALL be based on predicted student errors with specific corrective feedback
- **AND** the alternatives SHALL include numerically distinct values that avoid trivially obvious wrong answers
